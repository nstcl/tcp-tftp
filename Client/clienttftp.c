  ...
  int file_exists (char * fileName)
{
  ...
  /*void sendMsg(int opcode,char* fileName,char[] data,int numBlock)
{
       char* buf = NULL;
       char* mode = NULL;
       char* txBuffer = NULL;
       ...
       int i=0;
            ...
            {
                  buf = malloc((strlen(fileName)+1)*sizeof(char));    
                   if(buf==NULL)
                   {
                     printf("error 5\n");    
                   }
...
/*void sendTCPMessage()
{
  char  msg[20];
  strcpy(msg,"01aaa");
if((numbytes = sendto(sockfd,msg, strlen(msg), 0, (struct sockaddr *)&dest_addr, sizeof(struct sockaddr))) == -1)
{
...
return 0;
  ...
  /*int  translateCommand(char *userCmd,char *command,char** parametres )
 {
     int i=0;
     int j=0;
     char param[MAX_CMD];
   ...
   trim(param);
   i=j=0;
   for(j=0;j<param[j]!=NULL;j++);//run to end of array
   if(param[0]==',' || param[--j]==',')
   {
    printf("invalid command\n");
        return 1;//error
